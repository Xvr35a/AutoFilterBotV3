...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        btn = []
        search = message.text
        mo_tech_yt = f"**💕 NAME:** {search}\n**🥰Type**:Mp4,zip,mp3,app**🍭Uploaded in  {message.chat.title}**"
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAACAgUAAxkBAAERXzZi8jm1Pqdihl0AAfITSK9Nvl')
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        await query.answer("You want scene pack request you👀",show_alert=True)
